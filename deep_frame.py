...
def run(exp_dir: str, num_layers: int, img_path: str, logger: logging.Logger):
    ...
    for epoch in range(num_epochs):
        ...
        # compute f_diff, f_tgt, f_syn
        f_tgt = model(img_tgt).sum()
        f_syn = model(img_syn).sum()

        likelihood = f_tgt - f_syn
        ...
        if (epoch + 1) % log_interval == 0:
            ...
            logger.info(f"Epoch {epoch+1:<4d}: likelihood = {likelihood:<10.2f}, f_tgt = {f_tgt:<10.2f}, f_syn = {f_syn:<10.2f}")
...
if __name__ == '__main__':
    # model = Descriptor(num_layers=3)
    # print(model.feat_size_list)
    # exit()
